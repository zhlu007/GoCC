...
for times = 1:1  %runing times
    ...
    original_data = importdata('Gowalla_edges.txt','\t');

    OrData = original_data.data;

    OrData = OrData + 1;
    
% fname = cellstr(ls('nodes_*.csv')); 
...
%%%%%%%%%%%%%%%%%%%%%%%% user scale setting %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for name_index = 1 : num_user(impact_index)
        ...
        users = str2num (user_scale{f_name});
        
        experimental_data =  randperm(users);%% randomly select users from the original dataset

        ExpData = zeros(users, users);

        for i = 1 : users

            experimental_data(i);

           temp = OrData(find(OrData(:,1) == experimental_data(i)),2);

           joint_set = intersect(temp', experimental_data);

           for j = 1 : length(joint_set)

               index = find(experimental_data == joint_set(j));
        ...
        graph_network = UserUser;
        
        graph_network=(graph_network ~= 0);
         
        graph_network(sum(graph_network, 1) < 2, :)=0;
